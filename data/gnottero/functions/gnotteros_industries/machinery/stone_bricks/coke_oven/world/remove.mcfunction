# Removing the barrel, killing the placeholders and killing the executor
    ...
    execute if block ~ ~ ~ barrel run setblock ~ ~ ~ stone_bricks destroy
    ...
    execute if entity @e[type=item,nbt={Item:{id:"minecraft:barrel"}},distance=..1] run kill @e[type=item,nbt={Item:{id:"minecraft:barrel"}},distance=..1]
    kill @s